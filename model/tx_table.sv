...
module tx_table (
    ...
    always_comb begin
        ...
        if (row_sel == ROW_NORMAL) begin
            if (subset_is_odd) table_entry = TX_TABLE_NORMAL_ODD[subset_col][Sd_n[5:0]];
            else               table_entry = TX_TABLE_NORMAL_EVEN[subset_col][Sd_n[5:0]];
        end else if (fixed_special_row) begin
            table_entry = TX_TABLE_SPECIAL_EVEN[row_sel][2'd0];
        end else begin
            if (subset_is_odd) table_entry = TX_TABLE_SPECIAL_ODD[row_sel][subset_col];
            else               table_entry = TX_TABLE_SPECIAL_EVEN[row_sel][subset_col];
        end
